...
class TestAuthenticationBusinessLogic:
    ...
    @pytest.mark.asyncio
    async def test_duplicate_email_registration_blocked(
        ...
        user_data_1 = UserCreate(
            username="user1", email="test@test.com", password="TestPass456$"
        )
        ...
        assert (
            "email already exists or conflicts with existing data"
            in exc_info.value.message
        )
        assert exc_info.value.details["email"] == "test@test.com"

    @pytest.mark.asyncio
    async def test_duplicate_username_registration_blocked(
        self, auth_service: AuthService
    ):
        """
        ...
        user_data_1 = UserCreate(
            username="testuser", email="user1@test.com", password="TestPass456$"
        )
        ...
        assert exc_info.value.status_code == 409
        assert (
            "username already exists or conflicts with existing data"
            in exc_info.value.message
        )
    ...
    async def test_login_with_wrong_password_fails(self, auth_service: AuthService):
        ...
        user_data = UserCreate(
            username="logintest", email="login@test.com", password="CorrectPass456$"
        )
        ...
        login_data = UserLogin(
            email="login@test.com", password="WrongPassword789#"  # Incorrect password
        )
    ...
    async def test_inactive_user_cannot_login(
        ...
        login_data = UserLogin(
            email="inactive@test.com", password="TestPass456$"  # Correct password
        )
