...
class TestSampleCRUD:
    ...
    async def test_create_sample_with_valid_data(
        ...
        assert created_sample.sample_type == SampleType.BLOOD, "Sample type should match input"
        assert created_sample.subject_id == "P999", "Subject ID should match input"
        assert created_sample.collection_date == date(2024, 3, 15), "Collection date should match input"
        assert created_sample.status == SampleStatus.COLLECTED, "Status should match input"
        assert created_sample.storage_location == "freezer-1-rowC", "Storage location should match input"
        
        # Verify UUID fields are generated
        assert created_sample.id is not None, "Sample record ID should be generated"
        assert created_sample.sample_id is not None, "Sample tracking ID should be generated"
        ...
        retrieved_sample = await sample_service.get_sample_by_id(created_sample.id, test_user1)
        assert str(retrieved_sample.id) == str(created_sample.id), "Retrieved sample should match created sample"
    ...
    async def test_update_sample_fields(
        ...
        created_sample = await sample_service.create_sample(original_sample_data, test_user1)
        ...
        assert updated_sample.sample_type == SampleType.TISSUE, "Sample type should be updated"
        assert updated_sample.subject_id == "P777", "Subject ID should be updated"
        assert updated_sample.status == SampleStatus.PROCESSING, "Status should be updated"
        assert updated_sample.storage_location == "freezer-2-rowA", "Storage location should be updated"
        
        # Verify immutable fields remain unchanged
        assert updated_sample.collection_date == date(2024, 2, 10), "Collection date should remain unchanged"
        assert str(updated_sample.id) == str(created_sample.id), "Record ID should remain unchanged"
        assert str(updated_sample.sample_id) == str(created_sample.sample_id), "Sample tracking ID should remain unchanged"
        ...
        assert updated_sample.updated_at >= original_updated_at, "Updated timestamp should be newer or equal"
    ...
    async def test_update_preserves_user_isolation(
        ...
        retrieved_sample = await sample_service.get_sample_by_id(updated_sample.id, test_user1)
        assert retrieved_sample is not None, "Original user should still have access after update"
    ...
    async def test_delete_existing_sample(
        ...
        sample_exists_before = await sample_service.get_sample_by_id(sample_id, test_user1)
        ...
        assert "success" in delete_result["message"].lower(), "Delete message should indicate success"
        ...
        assert str(sample_id) in str(exc_info.value), "Error should include the sample ID"
    ...
    async def test_get_sample_by_id(
        ...
        assert str(retrieved_sample.sample_id) == str(created_sample.sample_id), "Sample tracking IDs should match"
        
        # Verify all business data fields
        assert retrieved_sample.sample_type == expected_data.sample_type, "Sample type should match"
        assert retrieved_sample.subject_id == expected_data.subject_id, "Subject ID should match"
        assert retrieved_sample.collection_date == expected_data.collection_date, "Collection date should match"
        assert retrieved_sample.status == expected_data.status, "Status should match"
        assert retrieved_sample.storage_location == expected_data.storage_location, "Storage location should match"
        ...
        assert retrieved_sample.updated_at >= retrieved_sample.created_at, "Updated timestamp should be >= created timestamp"
        
        # Verify data types are correct
        assert isinstance(retrieved_sample.sample_type, SampleType), "Sample type should be enum"
        assert isinstance(retrieved_sample.status, SampleStatus), "Status should be enum"
        assert isinstance(retrieved_sample.collection_date, date), "Collection date should be date object"
    ...
    async def test_sample_not_found_error(
        ...
        assert str(non_existent_id) in str(error), "Error should include the requested ID"
        ...
        assert "Sample" in str(exc_info.value), "Update error should indicate Sample not found"
        assert str(non_existent_id) in str(exc_info.value), "Update error should include the sample ID"
        
        # Test with delete operation on non-existent sample
        with pytest.raises(NotFoundError) as exc_info:
            await sample_service.delete_sample(non_existent_id, test_user1)
        
        assert "Sample" in str(exc_info.value), "Delete error should indicate Sample not found"
        assert str(non_existent_id) in str(exc_info.value), "Delete error should include the sample ID"