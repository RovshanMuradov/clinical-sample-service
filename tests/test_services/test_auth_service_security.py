...
class TestPasswordSecurity:
    ...
    async def test_password_hashing_and_verification(self):
        ...
        assert hashed_password != plain_password, "Password should be hashed, not stored as plain text"
        ...
        assert verify_password(plain_password, hashed_password), "Password verification should succeed"
        assert not verify_password("WrongPassword", hashed_password), "Wrong password should fail verification"
        
        # Verify salt randomization - same password should produce different hashes
        hashed_password2 = get_password_hash(plain_password)
        assert hashed_password != hashed_password2, "Same password should produce different hashes due to salt"
        
        # Both hashes should verify the same password
        assert verify_password(plain_password, hashed_password2), "Second hash should also verify correctly"
        ...
        assert not verify_password(plain_password, "invalid_hash"), "Invalid hash should fail gracefully"
    ...
    async def test_password_verification_edge_cases(self):
        ...
        assert not verify_password(valid_password, corrupted_hash), "Corrupted hash should fail safely"
...
class TestJWTSecurity:
    ...
    async def test_jwt_token_creation_and_validation(self):
        ...
        assert decoded_payload["sub"] == user_data["sub"], "Token should contain correct user ID"
        assert decoded_payload["email"] == user_data["email"], "Token should contain correct email"
        assert decoded_payload["username"] == user_data["username"], "Token should contain correct username"
        assert decoded_payload["is_active"] == user_data["is_active"], "Token should contain correct active status"
        ...
        assert exp_datetime > datetime.now(timezone.utc), "Token expiration should be in the future"
        ...
        assert exp_datetime <= max_expected_exp, "Token expiration should not exceed configured time"
    ...
    async def test_jwt_token_with_custom_expiration(self):
        ...
        assert abs((exp_datetime - expected_exp).total_seconds()) < 60, "Custom expiration should be respected"
    ...
    async def test_expired_token_rejection(self):
        ...
        assert exc_info.value.status_code == 401, "Expired token should return 401 Unauthorized"
        assert "Could not validate credentials" in exc_info.value.detail, "Error message should indicate credential failure"
        assert "WWW-Authenticate" in exc_info.value.headers, "Should include WWW-Authenticate header"
        assert exc_info.value.headers["WWW-Authenticate"] == "Bearer", "Should specify Bearer authentication"
    ...
    async def test_invalid_token_rejection(self):
        ...
        for invalid_token in invalid_tokens:
            ...
            # Verify correct error response
            assert exc_info.value.status_code == 401, f"Invalid token '{invalid_token}' should return 401"
            assert "Could not validate credentials" in exc_info.value.detail, "Error should not leak token details"
            
            # Test decode_token returns None (doesn't raise)
            decoded = decode_token(invalid_token)
            assert decoded is None, f"decode_token should return None for invalid token '{invalid_token}'"
    ...
    async def test_token_with_wrong_secret_key(self):
        ...
        assert decode_token(wrong_secret_token) is None, "Token with wrong secret should be rejected"
    ...
    async def test_token_with_wrong_algorithm(self):
        ...
        assert decode_token(wrong_algo_token) is None, "Token with wrong algorithm should be rejected"
...
class TestAuthServiceSecurity:
    ...
    @pytest.mark.asyncio
    async def test_get_current_user_by_token_with_invalid_tokens(self, auth_service: AuthService):
        ...
        for invalid_token in invalid_tokens:
            ...
            assert user is None, f"Invalid token '{invalid_token}' should return None user"
        ...
        for malformed_token in malformed_tokens:
            ...
            assert user is None, f"Malformed token '{malformed_token}' should return None user"

    @pytest.mark.asyncio
    async def test_authenticate_user_security(self, auth_service: AuthService, test_user1):
        ...
        for edge_case_email in edge_case_emails:
            ...
            assert user is None, f"Non-existent user '{edge_case_email}' should return None safely"
            
        # Test that wrong credentials are handled securely
        # Test non-existent email returns None
        user = await auth_service.authenticate_user(UserLogin(email="nonexistent@test.com", password="password"))
        assert user is None, "Non-existent email should return None"
        
        # Test wrong password with existing email returns None
        user = await auth_service.authenticate_user(UserLogin(email=test_user1.email, password="wrongpassword"))
        assert user is None, "Wrong password should return None"
        
        # Test correct credentials return user
        user = await auth_service.authenticate_user(UserLogin(email=test_user1.email, password="testpass123"))
