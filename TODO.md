...
🕒 **Время выполнения**: ~30 минут
- [ ] **test_create_sample_with_valid_data()**
  ```python
  # Создать sample с валидными данными
  # Проверить что все поля сохранились корректно
  ```
- [ ] **test_update_sample_with_valid_data()**
  ```python
  # Создать sample, обновить поля, проверить изменения
  # Убедиться что user_id не изменился
  ```
- [ ] **test_delete_existing_sample()**
  ```python
...
- [ ] **test_pagination_validation()**
  ```python
  # skip < 0, limit > 1000, limit <= 0
  # Должны возвращать ValidationError
  ```
- [ ] **test_invalid_uuid_handling()**
  ```python
...
- [ ] **Data Isolation тесты проходят** (5 тестов) - КРИТИЧНО!
- [ ] **Authentication Security тесты проходят** (4 теста) - КРИТИЧНО!
